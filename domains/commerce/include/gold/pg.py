...
def execute_values(cur, sql: str, rows: list[tuple], page_size: int = 2000, fetch: bool = False):
    """psycopg2.extras.execute_values 래퍼(배치 insert). 대용량 초기 적재 라운드트립 감소.

    fetch=True 면 RETURNING 결과 전체를 리스트로 반환(entity_seq 매핑처럼 즉시 필요한 경우).
    """
    from psycopg2.extras import execute_values as _ev

    return _ev(cur, sql, rows, page_size=page_size, fetch=fetch)
