...
### 86. 운영 기록 단일 관문 + ops 존 → 조회 DB(D1) 적재 (ASK-Seoul#78)
...
- **`P-1`·`P-2` 는 이미 적용돼 있다 — #78 §2 「현재 어긋난 곳」 표가 최신이 아니다.** commerce
  ...
  파티션을 파생하며, `%Y/%m/%d` 표기는 코드에 없다. 구 레이아웃 오브젝트 4,771개도 그때
  `load_date=` 파티션으로 복사 이관했다(총 5,075개, 실패 0). 이관이 소스 무삭제였고 dev 버킷을
  롤백용으로 보존했으므로 `seoul-dev` 에 옛 경로가 남아 있을 수 있으나, 그건 쓰는 쪽 규칙 위반이
  아니라 `Y-2`(읽는 쪽 0인 방치 자산) 정리 대상이다(잔량 미실측). #78 에 표 수정 요청함.
