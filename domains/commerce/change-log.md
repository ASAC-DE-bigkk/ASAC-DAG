...
각 항목은 세 칸으로 쓴다 — `request:`(요청·확정) · `response:`(한 일) · **`decision:`(검토했다
안 하기로 한 것과 이유)**. 최종 반영된 형태로 정리하되, **중간에 되돌린 단계는 지우는 게 아니라
`decision:` 으로 옮긴다** — 접는 건 읽기 편하자는 것이지 시도한 걸 없애자는 게 아니다.
서식 근거·`decision:` 을 넣은 경위: `CLAUDE.md` §19 Change Log Rule (2026-08-02 개정).
...
request:
- Serving 게이트웨이 이슈 3건(ASK-Seoul-Serving#178·#179·#180)이 commerce 패턴 선언으로
  소급됐다. 이쪽이 고쳐야 하고 고칠 수 있는 범위는 전부 조치하고 이슈에 회신하되, 패턴
  표기 규약 정리는 후속 작업으로만 남기라는 지시.

response:
- #178: dbt `_commerce_gold__models.yml` 의 pattern_id 77건(74종 — 3종은 두 모델 중복
  선언)을 하이픈→밑줄 개명(ASAC-DBT#470). question_ko 의 축반전 쌍 상호참조도 함께 치환.
  개명 후 275건 전건 `^[a-z0-9_]{1,64}$` 적합, (모델,id) 중복 0. 외부 참조는 저장소 전수
  스캔으로 `verify_usage_patterns.py` `_PARAM_OVERRIDES` 키 4건뿐임을 확인하고 동기
  수정(ASAC-DAG#738).
- #179: churn_yearly 패턴 8건의 박힌 값(`y='2025' /* :y */` 형)을 실제 바인딩(`y = :y`)으로
  전환, 예시값은 같은 모델 `inv_*` 3건이 이미 쓰던 선행 주석 문법(`-- :y='2025', :n=10`)으로
  이동. `gu_regime_diagnosis` 는 서브쿼리 2곳의 y 도 `:y` 로 통일.
- 재검증: verify 스크립트에 `--only`(key 부분 일치) 추가 후 `--apply --only d1_churn_yearly/`
  로 실 D1 재실행 — 12/12 verified, row_drift 0(예시값 바인딩 결과가 종전 하드코딩 결과와
  행수 동일 = 등가 전환 실측). `verified_at`/`verified_publication_id` 갱신.
- #180(glossary 발견 불가)은 코드 무변경 — 계약 사안이라 이슈 코멘트로 회신(아래 decision).

decision:
- #178 B안(게이트웨이가 하이픈 허용)은 기각 — 하이픈은 규약이 아니라 최초 선언 커밋
  (e89f8f0) 안에서 밑줄 198건과 섞여 들어간 표기 드리프트임을 이력으로 확인했다. 게이트웨이
  완화는 normalizeIntent 값-최소화 결정(Serving decision/0001)을 건드리고 도메인 간 표기
  분열을 굳힌다. 옛 id 는 전부 400 이라 소비 파손이 없어 리다이렉트 게시도 두지 않는다.
- 전 275건 재검증은 하지 않았다 — id 개명은 SQL 불변이라 기존 증거를 무효화하지 않는다.
  재검증은 SQL 이 바뀐 churn_yearly 로 한정(같은 모델 잔여 4건은 스탬프 일관성 겸 동반).
- #180 A안(columns[].vocabulary_id 선언)은 동의하되 선반영하지 않는다 — `_catalog` columns
  JSON 은 전 도메인 동형 유지가 선행 결정이고, 공유 핸드오프 표에 컬럼을 더하면 구 실행기가
  스키마를 되돌리는 왕복이 재현된다(#706 에서 display 를 새 표 `d1_catalog_display` 로 낸
  이유와 동일). 계약 필드가 확정되면 dbt 선언·export 게시를 붙인다.
- 표기 규약(슬러그·바인딩·예시값 주석) 문서화는 수리와 분리해 후속 이슈(ASAC-DBT#471)로
  남겼다 — 규약 문서가 먼저, CI 기계 검사는 그 다음 판단.

## 2026-08-07

### 100. 스키마 관문(schema_guard) — 원천 키 변경을 정리 단계에서 감지·격리한다

request:
- #732 사고의 재발 방지 기능을 요청했다: raw 수집 후 정리 단계에서 키 변경을 감지하고,
  감지 시 처리 중단 + Discord 경고 + 파일 격리로 브론즈·실버·골드 전파를 차단하며,
  변경사항을 신속히 조치할 수 있어야 한다.

response:
- `bronze/schema_guard.py` 신설: 수집 완료 후 정렬·diff 로 가기 **전에** 표본(첫·끝 페이지)의
  **정본화 이후** 키셋을 기준선 사이드카(diff-target 레이어 `<short>.schema_keys.json`)와 대조.
  별칭표가 아는 개명(v2 12종)은 정본화가 흡수해 통과하고, 별칭표 밖 변화만 걸린다.
- 감지 시: diff-target·증분 무변경, 원문(정본화 전 키 그대로)을 run 폴더 `_quarantine/` 에
  보존, `status=quarantined` → incomplete 마커(비게시 — 브론즈 적재가 소비하지 않아 하류
  전파 불가), §19.1 규격 Discord 경보(error, 추가/제거 키·격리 위치·조치 절차 포함).
- 조치 경로: 개명이면 별칭표 확장(#732 값 검증 절차), 정당한 변화면
  `scripts/schema_guard_accept.py --apply`(dry-run 기본, 직전 기준선을 문서 안 `previous` 로
  보존). 격리 run 의 변경분은 diff-target 이 안 움직였으므로 다음 정상 수집의 diff 가
  재흡수한다 — 유실 없음.
- 기준선은 **성공 적재 직후에만** 전진(부트스트랩 포함) — 실패 run 이 기준선을 오염하지
  않는다. 배포 후 첫 수집이 152종 기준선을 부트스트랩한다.
- 테스트 6종(감지 양방향·별칭 흡수 오탐 방지·격리 시 store 미호출·completed 마커 부재·
  기준선 전진·부트스트랩) — 전체 501 passed · security PASS.

decision:
- **전 행 키 검사안은 기각** — mail_order 94만행 재파싱이 수집 지연을 키운다. LOCALDATA 는
  시대 안에서 행 간 스키마가 고정이라 첫·끝 페이지 표본으로 충분하다(08-04 사고 패턴은
  응답 전체 개명이라 어느 페이지든 드러난다).
- **기준선을 직전 diff-target 에서 매번 유도하는 안은 기각** — 대형 타깃(수십만 행) GET 이
  관문 비용을 키운다. 사이드카는 수 KB 이고 적재 성공과 원자적으로 갱신된다.
- **DAG 전체 중단이 아니라 데이터셋 단위 격리** — 한 데이터셋의 원천 변경이 나머지 151종
  수집을 막을 이유가 없다(§19.1 격리 원칙과 동일). run 요약에는 quarantined 가 비-ok 로
  집계돼 `_RUN.incomplete` 로 드러난다.
...
request:
- ASAC-DBT PR#436 내용에서 메타데이터 서빙 규약이 빠졌고, ASAC-DBT#434 가 관련 문제를 제기하는
  것으로 보인다. **이전 메타데이터 적재 약속과, 왜 지워졌는지, 그 액션이 왜 진행됐는지 이력을
  파악하고 원인 분석부터 할 것.** 개발 작업은 먼저 착수하지 말 것.
- 원인 확인 후 — 그것을 바탕으로 브랜치 따서 수습할 것.

response:
- **약속 확인**: ASAC-DAG#478 Serving Contract v1(도메인별로 갈린 서빙 메타를 통일하는 org 계약)에
  2026-07-22 07:29 commerce(Exisign)가 **"전체 동의합니다"** 로 명시 동의. 계약 조항은
  "기존 필드와 `meta.serving` 을 장기간 이중 선언하지 않는다, 병합 후 `meta.serving` 만 정본".
  #478 은 2026-07-30 `COMPLETED` 로 닫혔다.
- **실측(2026-08-05)**: gold 22종이 #478 v1 필수 7종(`enabled`·`product_id`·`contract_version`·
  `grain`·`primary_key`·`publication_mode`·`zero_policy`)을 **22/22** 선언. v1.1
  `publication_trigger` 22 · `event_time` 3 · `freshness_slo_minutes` 3(조건부 필수 충족).
  `external` 22. `publication_mode: snapshot` 22/22 로 #478 enum 안. `serving_export.py:955` 가
  `meta.serving` 을 읽어 `_catalog` 를 채운다. 운영 D1 `_catalog` commerce 22행 전부 `external=1`.
  **즉 commerce 는 #478 을 이미 채택한 상태다.**
- **문서 4곳 정정**: `CLAUDE.md §19.2` · `docs/serving-contract-chain.md`(전면 재작성) ·
  `docs/README.md` · `Share.md`. 실측 필드 표, 확장 필드의 위치 규칙(같은 블록), `external: false`
  로 외부 공개를 내리는 절차(#434 방식), 그리고 **왜 틀린 문서가 생겼는지 이력**을 남겼다.

decision:
- **왜 지워졌나 — 중복 적용의 과잉 교정.** 2026-07-23 에 이미 `meta.serving` 이 적용돼 있었는데
  07-27 에 같은 내용을 한 번 더 적용해(586446b) `config.meta.serving` 블록이 충돌했다. 중복을
  되돌린 것(08f6f88)까지는 **맞았다.** 문제는 그 김에 거버넌스 문서까지 "commerce 는 #478 에 강제
  종속되지 않는다"로 다시 쓴 것(a61c4dc)이다 — 실제 충돌 범위는 YAML 한 파일인데 결론은 org 계약
  비적용까지 갔다. 하루 뒤 PR#335 머지·22종 재작성으로 코드는 정반대로 갔고 문서만 남았다.
  → **로컬 충돌은 충돌만 되돌린다. 거버넌스 문서를 함께 고치지 않는다**를 문서에 명시했다.
- **PR#436 을 닫지 않고 그 자리에서 고쳤다.** 아직 `OPEN` 이라 틀린 문서가 dev 에 들어가기 전이었다.
  새 브랜치로 사후 수습하는 것보다 애초에 들어가지 않게 하는 편이 깨끗하다고 봤다.
- **`serving_tier` 를 걷어내지 않았다.** #478 이 금지한 것은 *다른 이름의 규약을 병행 선언하는 것*
  이지 확장 자체가 아니다. `serving_tier`·`d1_table` 은 같은 `meta.serving` 블록 안의 확장이고
  export 가 실제로 소비하므로 유지한다. 별도 네임스페이스(`meta.commerce_serving`) 분리안은
  이중 선언으로 되돌아가는 것이라 채택하지 않았다.
- **이전 판의 CI 게이트 경고는 삭제가 아니라 반증으로 남겼다.** "필수필드 없음 / `publication_mode`
  가 enum 밖이라 게이트에 걸릴 수 있다"는 경고가 실측과 어긋나므로, 지우지 않고 **대조표**로 남겨
  왜 무효인지 보이게 했다.
- **#434 회신은 하지 않았다.** 사용자가 데이터를 직접 확인한 뒤 함께 보기로 했다. commerce 22종은
  활용 예시 0건이 없어 정리 대상은 아니지만, 회신 문안은 확인 후 작성한다.

## 2026-08-04

### 95. 상태 경로 이관 누락 → 152종 전량 재수집 → silver 가 메모리로 못 넘는 상태 (#699)

request:
- 로컬 실버가 매번 같은 지점에서 멈춘다. 실제 필요 메모리를 재서 제안할 것.
- 상위 프로젝트 설정은 건드리지 말고 번들 안에서 해결할 것. 이미 최적화된 값을 더 올리지 말 것.
- 골드 생성 등 후속 실행 형태가 바뀌면 안 된다. **실버의 결과 형태 자체가 달라져선 안 된다** —
  적용 전후 실행 결과가 동일해야 한다.
- 왜 전량 수집 run 이 생겼는지 다시 검토하고, **결과 보고가 아니라 이슈 문서로** 작성할 것.
- "나 이거 진짜 난점이다" — change-log 에 남길 것.

response:
- **원인**: ops 존 경로 전환(`COMMERCE_*_STATE_LAYER`, #62/#78) 시 **기존 상태 파일을 새 경로로
  이관하지 않았다.** 코드 기본값은 지금도 옛 경로(`commerce_bronze_state`)이고 경로를 정하는 건
  `.env` 뿐이라, 값만 바꾼 순간 새 경로가 비어 보였다. 실행 로그에
  `no_watermark=True, units=152, lookback=∞일` 이 그대로 남아 있다. 워터마크가 없으면
  `wm=""` 이라 `r > wm` 을 모든 run 이 통과하고 lookback 까지 풀려 제동 장치가 사라진다
  → bronze 전량 1건(2,900,052행 / 2,690,747키 / 152종) → silver 증분의 `affected_keys` 가
  전체 키를 덮음 → `prior_tail` 이 history 280만 행을 통째로 스캔 → `TopNRankingOperator` 폭발.
- **유실이지 부트스트랩이 아님을 확정**: 그 run 의 152종 **전부** bronze 최초 관측이 08-04 이전이다
  (진짜 최초 0종). 앞선 full run 5건(06-30 39종, 07-09 93종, 07-14 7종=v2 purge 재수집,
  07-17 7종, 07-18 2종)은 신규 등록분의 정상 최초 적재였다.
- **이관 누락 확정 — 옛 경로 파일이 지금도 버킷에 살아 있다**: 버킷 전수 스캔 결과
  `commerce_bronze_state/_watermark.json`(07-27 19:06, 7570B) ·
  `commerce_silver_state/_watermark.json`(07-27 20:09, 129B) 가 그대로 있고,
  새 경로 bronze 파일은 크기까지 같은 7570B 인데 **08-04 19:02 자 = 전량 수집이 끝난 뒤 새로
  쓰인 것**이다. 새 경로 silver 워터마크는 **아직도 없다**. 삭제가 아니라 이관 누락이다.
- **타임라인**: 07-27 옛 경로 마지막 기록 → 07-28 `.env.example` 에 ops 존 블록 추가(`08269be`,
  #62) → 07-29·07-30 정상 증분(`no_watermark=False`, units 67/76, pending 151) → 로컬 `.env`
  반영 → **08-04 12:04 `no_watermark=True`, units 152, pending 0** → 08-04 19:02 새 경로 최초 기록.
  `pending` 이 151→0 으로 떨어진 것도 `_pending.json` 이 같이 안 옮겨졌기 때문이다.
- **silver 도 같은 사고를 안고 있다**: 옛 경로 silver 워터마크의 `max_collected_at` 이
  `2026-07-28T00:34:13` 로, 현재 `silver_license_history` 최대 수집 시각과 정확히 일치한다.
  상태 4종(bronze·silver·serve·watchdog) 전부 같은 누락에 노출돼 있다.
- **메모리로는 못 넘는다는 것을 실측으로 확정**: 상한 800MB→0.60GB(75%), 2GB→1.82GB(91%),
  2.8GB→2.56GB(91%), 4GB→3.81GB(95%) — **전부 실패, 준 만큼 그대로 씀**. 수렴하는 상한이 없다.
  spill 도 답이 아니다(Trino spill 은 aggregation/join-build/order-by/window 만 덮고
  `TopNRankingOperator` 는 대상이 아니다). ASK-Seoul#94 의 "spill 켰는데 터진다"와 같은 이유다.
- **밀린 run 은 버릴 수 없음을 실측**: distinct hash 2,899,812 중 이미 history 에 있는 것
  1,791,943(61.8%), **없는 새 내용 1,107,869(38.2%)**. 워터마크만 밀면 그만큼 유실된다.
  분할 처리가 유일한 경로 — 152종 평균 19,079행, 50k 초과 10종, 200k 초과 2종,
  **최대 1종 937,245행(전체의 32%)** 이라 그 1종만 추가 분할하면 된다.
- 조치안 6건(경로 전환 시 상태 이관, 부재/부트스트랩 구분 가드, 복구 lookback 상한, 조용한 빈
  dict 경로 분리, 상태 파일 사본, 밀린 run 분할)과 운영 위험 판정을 **ASAC-DAG#699** 로 발행.

decision:
- **모델 SQL 수정은 되돌렸다.** `prior_tail` 을 손대 메모리를 줄이는 안을 실제로 넣었다가
  전량 폐기했다 — 히스토리 값 변화를 무시하게 되어 **적용 전후 결과가 달라진다**. 사용자가
  "데이터 정합성을 무시하라는 말이 아니다 / 되돌리고 원점부터 검토해라"로 반려. 메모리 절감보다
  결과 동일성이 우선이라는 판단이 확정됐다. 현재 모델은 HEAD 그대로다.
- **메모리 증설(안 A)은 폐기.** 12g/4GB 까지 올려 한 번만 통과시키려 했으나 실패. 위 4단계
  실측이 "필요량이 데이터 크기에 비례할 뿐 수렴하지 않는다"를 보였으므로 증설은 방향 자체가 틀렸다.
  상위 프로젝트 커밋값(per-node 800MB)으로 원복했고 임시 override 는 커밋하지 않았다.
- **전량 run 폐기 + 워터마크 강제 전진**은 가장 싸지만 채택하지 않았다. 38.2% 가 history 에 없는
  새 내용이라 데이터 손실이 된다. 싸다는 이유로 유실을 택할 수 없다.
- **"삭제됐고 주체 미상"으로 먼저 결론냈다가 뒤집었다.** 접두 범위만 좁혀 보고
  `ops/control/state/commerce/` 하위가 전부 08-04 자인 것, 레포에 삭제 코드가 없는 것, 버킷
  수명주기에 만료 규칙이 없는 것(멀티파트 중단 7일뿐), R2 버저닝이 꺼져 있어
  `ListObjectVersions` 가 `NotImplemented` 인 것까지 확인하고 "수동 조작, 특정 불가"로 적었다.
  **틀렸다** — 버킷을 전수 스캔하니 옛 경로에 파일이 그대로 있었다. 좁힌 접두만 보고 "없다"를
  "지워졌다"로 읽은 게 원인이다. 초판 이슈 본문도 정정 주석과 함께 다시 썼다.
  (다만 R2 버저닝 부재는 그대로 유효한 문제라 조치안 (5) 사본으로 남겼다.)
- **상태 파일을 옛 경로에서 복원하지 않았다.** silver 워터마크를 새 경로로 옮기면 복원값이
  `2026-07-28` 인데 전량 run 의 `collected_at` 은 08-04 라 **이번 적체는 전혀 줄지 않는다.**
  반면 07-28~08-04 사이 수집분을 건너뛰게 만들 위험은 실재한다. 다음 사고를 막는 조치일 뿐이라
  단독 실행하지 않고 #699 §6(1) 로 넘겼다.
- **운영은 건드리지 않았다.** 이번 건은 `seoul-dev` 버킷이고 운영은 다른 버킷이라 전파 경로가
  없다. "이번 실버는 운영과 별건으로 작업하자"는 지시에 따라 맥미니 접근 없이, 확인·조치 항목
  4개를 #699 에 적어 운영 담당에게 넘겼다.
- 다만 위험 판정은 "가정"에서 **"예정된 사고"로 올렸다.** 경로를 정하는 게 `.env` 뿐이라
  운영이 아직 옛 경로면 **새 경로로 바꾸는 그 순간** 같은 전량 수집이 걸린다. 16GB 맥미니에서는
  Trino 가 먼저 죽고, 그게 ASK-Seoul#94 가 보고한 모양이다. 그래서 #699 는 "전환 전에 가드부터"
  를 요구한다.
- **전량 run 의 38% 가 새 내용이었다는 사실**은 증분 수집이 그동안 놓친 게 있다는 뜻일 수 있다.
  이번 건의 원인과는 별개라 파고들지 않고 #699 §8 에 미상으로 남겼다.
...
request:
- 프로젝트 전체 회고(4개 리포 이슈 103건 + 이 이력 90건 전수 대조)를 사람 중심 문서로 정리하고,
  난관을 **원인 > 난점 > 해결** 로 짚을 것. 이력이 없으면 없다고 명시하고 억지로 채우지 말 것.
- 회고에서 나온 서식 제안(`decision:` 칸 추가)을 **채택하고 CLAUDE.md 에도 명시**할 것.

response:
- 회고 문서 [`docs/project-retrospective.md`](../../../docs/project-retrospective.md) 신설(551줄,
  부모 리포 ASK-Seoul). 난관 **28건**을 6개 유형으로 분류하고 각 건에 **기록된 원인**을 붙였다.
  심층 6건은 원인·난점·해결 3단으로. 이슈 [ASK-Seoul#88](https://github.com/ASAC-DE-bigkk/ASK-Seoul/issues/88)
  (상시 갱신 기록장, 닫지 않음) · PR [ASK-Seoul#89](https://github.com/ASAC-DE-bigkk/ASK-Seoul/pull/89).
- **서식 개정**: `CLAUDE.md` §19 Change Log Rule 에 세 번째 칸 `decision:` 추가 + §18 Final Quality
  Gate 에 확인 항목 1줄. 이 파일 머리말도 3칸 서식으로 갱신.
- **"중간 단계는 접는다" 지시를 수정**: 기존 문구가 "fold superseded intermediate steps"(되돌린
  단계는 합쳐서 기술)라 **버린 시도를 지우라는 지시**로 작동하고 있었다. "접되 지우지 말고
  `decision:` 으로 옮긴다"로 바꿨다.

decision:
- **89 GiB 테이블(ASK-Seoul#78 `Y-7`) 드롭을 시행하지 않기로.** 실측 결과 prod 카탈로그에 `ops`
  스키마 자체가 없고 창고 전체가 38.27 GiB 라 89 GiB 객체가 존재할 수 없다. 소유도 commerce 가
  아니다(전 도메인 공통 실행기록 테이블, commerce 참조 0). 대상은 dev 카탈로그이고 그건 운영 이관
  종결 시까지 **롤백 지점으로 보존**하기로 한 자산이라, 시행 대신 #78 에 배정 정정을 요청했다.
- **`seoul-dev` 를 "폐기 대상"으로 적었다가 철회.** §88 초안에서 그렇게 썼으나 사용자 확인 결과
  운영 이관이 완전히 끝날 때까지 되돌아갈 지점이다. 문서 3종을 "동결된 롤백 지점"으로 정정했다.
- **citydata·culture 의 구 규약(두 키 세트 공존) 지원 코드는 건드리지 않기로.** 환경 지정 키
  제거(#654)를 전 리포에 적용했으나 이 둘은 자기 규약과 안전 테스트를 갖고 있어, 전환하면 그
  도메인의 자격증명 해석이 바뀐다. 유예 목록에 담당자·사유와 함께 남겼고 전환은 오너 판단이다.
  (한 번 강행했다가 culture 의 "구 규약 박스에서 prod 로 폴백 금지" 가드를 깨뜨려 되돌렸다.)
- **회고 전문을 이슈 본문에 넣지 않기로.** 이슈 본문 한도 65,536자 중 문서가 이미 44%를 쓰고,
  더 결정적으로 **본문은 편집해도 diff 가 남지 않는다**("edited" 표시만). 회고록의 값은 판단이
  바뀐 궤적인데 그게 안 남는다. 파일=정본(git 이 버전·diff), 이슈=요약+갱신 로그로 나눴다.
- **운영 기록 최초 전량 적재를 미루기로.** 22,589건을 한 번에 돌렸더니 30분 넘게 0행이었다.
  적재기가 전부 읽은 뒤에야 쓰는 구조라 진행이 안 보이고 중단 시 전량 폐기된다. 날짜 단위 분할
  커밋을 먼저 넣기로 하고 #655 에 결정 요청으로 올렸다. 평상시엔 무해해 정기 실행은 그대로 둔다.
- **TRINO_DEV_ICEBERG_CATALOG 정리를 처음엔 범위 밖으로 두려다 포함.** 타 도메인 dbt 타깃 해석을
  바꾸는 변경이라 오너 합의가 먼저라고 판단했으나, 사용자가 "해당되는 부분 모두"로 지시해 전환했다.
  대신 실코드만 바꾸고 자기 규약을 가진 2건은 위와 같이 유예했다.
