...
commerce 전용 인자는 **루트 `.env` 의 `commerce 전용값` 블록**이 공급하며 `.env.commerce` 가
코드 이름으로 매핑한다 → [configuration.md](configuration.md).
...
- 코드는 `get_storage()` 하나만 호출 — 단계 로직은 백엔드를 모른다. **백엔드 전환은
  루트 `.env` 의 `COMMERCE_STORAGE_BACKEND` 값만 바꾸면 끝**(코드 변경 없음).
...
1. **호스트 프로세스 env** — 루트 `.env`(`env_file: .env`)와 compose `environment:` 가 주입.
   Airflow/Postgres/Trino/dbt/R2-Iceberg **및 `commerce 전용값` 블록**(2026-07-28~)이 여기 있다.
...
> dev/prod 분리는 **버킷·토큰**으로 보장한다. 별도 컴포즈 파일이 아니라 루트 `.env` 의
> `R2_DEV_*`/`R2_*` 값(=`.env.commerce` 가 `R2_BUCKET` 등으로 매핑)으로 가른다.

## 코드 변경 없이 전환

```bash
# 루트 .env 의 'commerce 전용값' 블록에서 바꾼다
COMMERCE_STORAGE_BACKEND=r2
R2_DEV_BUCKET_NAME=seoul-dev   # prod 는 seoul-prod (.env.commerce 가 R2_BUCKET 으로 매핑)
