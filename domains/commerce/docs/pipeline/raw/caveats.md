...
| # | 특이사항 | 단계 | 대응 |
|---|---|---|---|
| C1 | **정렬 키 없음** — 페이징은 위치 기반, 응답에 정렬 컬럼/순번 없음. 수집 중 원천 변동 시 페이지 경계에서 누락/중복 가능 | `[bronze]` `[silver]` | bronze: 완전성 점검(수집건수=`list_total_count`)으로 부분 잡고 재수집. silver: `(OPNSFTEAMCODE, MGTNO)` 로 dedup |
...
| C9 | **좌표/선택 컬럼 공란** — `X`/`Y`, `SITEAREA`, `UPTAENM` 등 업종군별 누락 가능(v1 공통 14컬럼 외) | `[silver]` | optional 처리, 누락 허용 |
...
1. `(OPNSFTEAMCODE, MGTNO)` dedup(C1) · 날짜 `strip`/파싱(C2) · 상태코드 매핑(C5) · 선택컬럼 optional(C9)
