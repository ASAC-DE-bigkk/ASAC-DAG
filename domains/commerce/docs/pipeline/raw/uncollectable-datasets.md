...
**수집 대상 152종(v1 인허가 139 + v2 환경 13) 전부 해소됐다**(`enabled('daily')=152`, `pending=0`). 애초에
...
- **해소 경로(39종 시점)**: ① 식품군은 처음부터 자동 해석 → ② 의료·동물 13종은 BPLCNM 수동 확인(§2-1) →
  ③ 공중위생·축산·관광·건기식·숙박 **14종은 포털 정본 코드**로 확정 입력(§2-2). → 당시 **39/39** 완결.
  이후 대상 확장으로 **현재 152종(v1 139 + v2 13) 전부 수집, pending 0**.

> 집계: 레지스트리 **152종 = 수집 152 / 미수집 0**(v1 인허가 139 + v2 환경 13). (인허가 외 2종 격리로 154→152.
> 서울 LOCALDATA 인허가는 v1 139종보다 많다 — 단란주점·유흥주점·위탁급식·노래연습장 등이
...
실측(해소 후, 152종): **`enabled('daily')=152`, `pending('daily')=0`**. 즉 `ingest_one`
매핑이 152 인스턴스로 펼쳐진다(job 152개). 더는 스킵되는 데이터셋이 없다.
