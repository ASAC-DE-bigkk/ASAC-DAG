...
| `food_hygiene_status` | 서울시 식품위생업소 현황 | OA-13663 | irregular | `SeoulFoodHygieneBizHealthImport` | **'현황' 집계형** — 봉투는 호출되나 **53컬럼·~119만건, v1 공통 14컬럼(MGTNO 등) 없음** → silver 정규화 부적합 |

## 영향

- `monthly`/`irregular` 주기에는 인허가 대상이 **0종**만 남아, 해당 DAG(`seoul_commerce_monthly`·
  `seoul_commerce_irregular`)를 **비활성화**했다([../../commerce_raw.py](../../commerce_raw.py)
  의 `SCHEDULES` = `daily` 만). 현재 생성되는 DAG 는 **`commerce_collect_raw` 1개**.
- 수집 대상 레지스트리 집계: **154 → 152종**(전부 daily) = **152종 전 종 수집(미해석 0)**. (인허가 외 2종 격리 반영.)
...
2. **food_hygiene_status**: 서비스명은 `SeoulFoodHygieneBizHealthImport`. **v1 공통 14컬럼이 아니므로**
   bronze 전용 수집 또는 전용 silver 스키마를 신설해야 한다(현재 silver 는 v1 공통 14컬럼 정규화).
3. 인허가로 편입할 경우에만 `config/dataset_registry.yaml` 의 `datasets:` 에 항목을 옮기고,
   `monthly`/`irregular` 를 `SCHEDULES` 에 되살린다.

관련: [raw/uncollectable-datasets.md](raw/uncollectable-datasets.md)(R3 비-LOCALDATA) ·
[raw/caveats.md](raw/caveats.md)(스키마 상이) · [common_info.md](common_info.md)(카탈로그 152종).
