...
- **실행 1회 = 알림 1건. 대상 0종이어도 완료 알림을 보낸다**: 침묵은 "정상 0종"과 "수집이 멈춤"을
  구분해 주지 못한다(수신자는 둘 다 메시지 없음으로 관측). 0종 알림에는 **왜 0종인지**를 함께 싣는다
  — collect=동일 KST 일자 완료 종수 + 그 일을 끝낸 run_id, recollect=미완료 API 없음/일자변경 가드.
  0종을 동일자 완료로 다 설명하지 못하면 잔여분을 **미수집(⛔)** 으로 올려 경고색으로 띄운다
  (게이트·플랜 실패 같은 상류 스킵이 초록 0종에 묻히지 않게).
...
- 2026-07-31: **0종 실행도 완료 알림 발송(§2)** — 사용자 지시("수집 건수가 0건이더라도 실행 완료가
  ...
  이 리포트 전에 조기 return 해 **성공인데 알림이 없는 침묵 구간**이 생겼다(2026-07-31 실측 3 run).
  0종 알림에는 사유(동일자 완료 종수 + 완료 run_id)를 싣고, 동일자 완료로 설명되지 않는 잔여는
  미수집(⛔)·경고색으로 올린다.
