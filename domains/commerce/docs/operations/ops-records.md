...
- **포인터는 뒤늦게 채워진다.** 실행 기록은 태스크가 끝나는 즉시 쓰이고 번들은 그 run 이 종결된
  뒤 하루 1회 올라가므로, **기록이 먼저 적재되고 번들이 나중에 생긴다.** 적재 시점에만 붙이면
  이미 넣은 행은 다시 안 보므로 영영 비어 있게 된다 — 그래서 `common_ops_d1_load` 가 매 실행
  끝에 "포인터가 빈 행 중 지금은 번들이 있는 것"을 찾아 채운다(질의 1 + 배치 UPDATE, 번들이
  없으면 질의조차 안 한다).
