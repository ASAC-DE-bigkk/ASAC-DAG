...
def test_missing_description_is_reported_with_column_names(spec, monkeypatch):
    ...
    rows, _ext, _pat, _disp, _params = se._handoff_rows(spec, meta, COLS, "pub-1")
...
def test_publication_is_not_blocked_by_a_missing_description(spec, monkeypatch):
    ...
    _events(monkeypatch)
    rows, ext, _pat, _disp, _params = se._handoff_rows(spec, {"columns": {}, "serving": {}}, COLS, "pub-1")
...
def test_dbt_columns_win_nothing_when_derived_overrides(spec, monkeypatch):
    ...
    rows, _ext, _pat, _disp, _params = se._handoff_rows(spec, meta, [("share", "double")], "pub-1")
