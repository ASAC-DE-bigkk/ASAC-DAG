...
from traffic_ingest.reliability.lineage import (  # noqa: E402
    StagePolicy,
...
def _policy() -> StagePolicy:
    return StagePolicy("gold", "Gold", "traffic_gold_transform", 90)
...
def test_latest_success_with_recovered_failure_is_warn():
    ...
    assert summary["reason"] == "recovered_failure"


def test_stale_running_run_is_fail():
    summary = summarize_stage_runs(
        policy=_policy(),
        runs=[_run("RUNNING", "2026-07-19T00:00:00Z")],
        detected_at=DETECTED_AT,
        lookback_hours=24,
    )
...
def test_latest_terminal_failure_is_fail():
    summary = summarize_stage_runs(
        ...
        runs=[
            _run(
                ...
                ended_at="2026-07-19T00:32:00Z",
                duration_ms=120_000,
            ),
            _run(
                "ABORTED",
                "2026-07-19T01:30:00Z",
                ended_at="2026-07-19T01:31:00Z",
                duration_ms=60_000,
            ),
        ],
        detected_at=DETECTED_AT,
    ...
    assert summary["status"] == "FAIL"
    assert summary["latest_terminal_state"] == "ABORTED"
...
def test_collect_pipeline_stages_uses_exact_encoded_job_url():
    ...
    result = collect_pipeline_stages(
        policies=(StagePolicy("source", "Source", "job/name", 90),),
        detected_at=DETECTED_AT,
        lookback_hours=24,
    ...
    assert result["stages"][0]["key"] == "source"
