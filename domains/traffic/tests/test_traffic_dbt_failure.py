...
def test_marks_silver_persisted_when_first_silver_model_completed_before_failure():
    ...
    assert silver_persisted_from_results(results, default=False) is True
...
def test_builds_recovery_record_with_snapshot_contract_context():
    ...
    assert record["traffic_snapshot_dag_run_id"] == "scheduled__2026-07-13T01:35:00+00:00"
...
def test_recovery_record_sink_writes_a_run_scoped_r2_document():
    ...
    key = R2RecoveryRecordSink(put_object=lambda object_key, body: written.update(
        key=object_key, body=body
    )).write(record)
