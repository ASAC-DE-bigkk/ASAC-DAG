...
def test_history_key_uses_the_ops_observed_date_partition_without_domain_suffix():
    assert history_object_key(date(2026, 7, 20)) == (
        "ops/reports/traffic/type=reliability/observed_date=2026-07-20/"
...
def test_load_recent_history_prefers_observed_date_over_the_legacy_path():
    ...
    observed_key = (
        "ops/reports/traffic/type=reliability/observed_date=2026-07-19/"
        "pipeline-reliability-v2.json"
    )
    legacy_key = (
        "ops/reports/traffic/type=reliability/date=2026-07-19/domain=traffic/"
        "pipeline-reliability-v2.json"
    )
    storage = FakeStorage(
        {
