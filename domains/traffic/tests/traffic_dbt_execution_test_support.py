...
MODULE_PATH = Path(__file__).resolve().parents[1] / "traffic_dbt_execution.py"
...
def write_actual_artifacts(
    command: list[str], *, source_freshness: bool = False
) -> None:
    target = Path(option(command, "--target-path"))
    target.mkdir(parents=True, exist_ok=True)
    (target / "manifest.json").write_text("{}", encoding="utf-8")
    artifact_name = "sources.json" if source_freshness else "run_results.json"
    (target / artifact_name).write_text("{}", encoding="utf-8")
