...
| `nx` | `config/seoul_kma_grids.csv`의 `nx` | 예보 격자 X |
| `ny` | `config/seoul_kma_grids.csv`의 `ny` | 예보 격자 Y |
...
dev 기본 스케줄은 각 발표시각 20분 뒤인 `20 2,5,8,11,14,17,20,23 * * *`이다.
prod에서는 `ASK_SEOUL_KMA_DAG_SCHEDULE`을 명시하지 않으면 자동 스케줄을 만들지 않는다.

서울 coverage는 `config/seoul_kma_grids.csv`에 고정된 80개 grid를 기준으로 한다. 기존 단일 지점
env인 `ASK_SEOUL_KMA_NX`, `ASK_SEOUL_KMA_NY`, `ASK_SEOUL_KMA_PLACE_ID`는 기본 수집 범위를
결정하지 않는다.
...
```text
bronze/weather_forecast/kma_vilage_fcst/load_date=YYYY-MM-DD/nx=<nx>/ny=<ny>/YYYYMMDDTHHMMSSKST_base-<base_date><base_time>_<request_id>.json
```

예시:

```text
bronze/weather_forecast/kma_vilage_fcst/load_date=2026-07-01/nx=60/ny=127/20260701T105517KST_base-202607010800_<request_id>.json
```
...
| `place_id` | `kma_<nx>_<ny>` 형식의 grid 식별자 |
...
- 사용자 위치 WGS84를 어떤 방식으로 가장 가까운 `kma_<nx>_<ny>`에 매핑할지
