...
| 파일 | 역할 |
|---|---|
| `kma_vilage_fcst_bronze.py` | Airflow DAG 엔트리포인트. task 순서와 실행 흐름만 잡고 세부 로직은 domain package에 위임한다. |
...
| `.airflowignore` | `weather_ingest/`, `docs/`를 DAG 파일 스캔에서 제외하고 import 대상으로만 둔다. |

## 실행 흐름

```text
Airflow DAG
  -> KMA getVilageFcst 호출
  -> 응답 JSON 성공 여부 검증
