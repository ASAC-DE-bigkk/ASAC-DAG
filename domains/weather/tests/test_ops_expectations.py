"""weather 기대 주기 등록 — 코드 선언과 조회 DB 사본이 어긋나지 않는지 (ASK-Seoul#78 §9, ASAC-DAG#733).

값의 정본은 실제 DAG 파일의 ``dag_id=``(또는 ``DAG_ID =`` 류 상수) 선언이다(`S-1`). 이 테스트는
그 정본을 실제로 읽어 :mod:`weather_ingest.ops_expectations` 사본과 양방향 대조한다 — DAG 를
...
from weather_ingest import ops_expectations  # noqa: E402
...
# dag_id 상수/리터럴이 top-level DAG 파일이 아니라 여기 있다(build_kma_bronze_dag 계열이
# DAG_ID/RECOLLECT_DAG_ID/BACKFILL_DAG_ID 를 여기서 선언하고 top-level 파일이 값만 가져다 쓴다).
_EXTRA_DECLARATION_FILES = (BUNDLE / "weather_ingest" / "bronze_dag_support.py",)
