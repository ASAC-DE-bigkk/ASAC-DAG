...
def send_discord_message(message: str, webhook_url: str | None = None) -> bool:
    ...
    **채널 결정은 이 도메인 규약 그대로다** — `discord_webhook_url()` 의 우선순위
    (`ASK_SEOUL_DISCORD_WEBHOOK_URL` → `WEATHER_DISCORD_WEBHOOK_URL`)를 쓰고, 그것이
    ...
    if not url:
        ...
        return False
    return send_payload(_discord_payload(message), domain="weather", webhook=url)


def send_discord_report(report: dict[str, Any], webhook_url: str | None = None) -> bool:
    """카드 payload 전송 — `build_weather_discord_payload` 결과를 그대로(#692).
    ...
    if not url:
        ...
        return False
    return send_payload(build_weather_discord_payload(report), domain="weather", webhook=url)
