...
def build_weather_discord_payload(report: Mapping[str, Any]) -> dict[str, Any]:
    status = str(report.get("status") or "UNKNOWN").upper()
    source = report.get("source") if isinstance(report.get("source"), Mapping) else {}
    weather = (
        report.get("weather") if isinstance(report.get("weather"), Mapping) else {}
    )
    bottleneck = (
    ...
    source_value = (
        f"Base slots {weather.get('base_time_count', 0)}/"
        f"{weather.get('expected_base_time_count', 'unknown')} · "
        f"grid {weather.get('grid_slot_count', 0)}/"
        f"{weather.get('expected_grid_slot_count', 'unknown')}\n"
        f"Raw pages {weather.get('raw_object_count', 0)} "
        f"(+{weather.get('additional_raw_page_count', 0)} pagination)\n"
        f"Freshness {source.get('freshness_minutes', 'unknown')}m · "
        f"coverage {source.get('coverage_percent', 'unknown')}% · "
        f"publishable={source.get('publishability_ok', False)}"
    )
    if bottleneck:
        bottleneck_value = (
    ...
    embed = {
        "title": _truncate(
            f"Weather Pipeline Reliability · {report.get('report_date', 'unknown')}",
            256,
        ),
        "description": "기상청 단기예보 Bronze→Silver/Gold 24시간 SLO",
