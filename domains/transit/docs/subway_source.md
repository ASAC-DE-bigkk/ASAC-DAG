...
|---------|-----|------|----------|------------------|-----|
| `subway_arrival` | #1 도착 | **역** | **강남·잠실·사당** (3, 핵심 환승역) | ✅ **수집** | `SUBWAY_STATIONS` |
...
- **호출량**: target 당 1콜(이중 호출 없음). 스케줄 `*/20`(72런/일).
  - 현재(도착 3역만) → 1런 3콜 → **216콜/일**.
  - position 재개 시(9호선+3역=12콜) → **864콜/일 < 일일 1000건**.
