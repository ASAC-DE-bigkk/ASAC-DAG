...
호출 예산: 위치=호선당 1콜, 도착=역당 1콜.
#212 로 실시간 dataset 수집 스코프를 재배분: silver 가 소비하지 않는 subway_position 은
수집에서 제외한다(DAG 의 SOURCES 에서 주석 처리 — 코드·테이블·파서는 유지).
- 현재(도착 3역만) = 3콜 × */20(72런/일) = 216콜/일.
- position 재개(SOURCES 주석 해제) 시 +9호선 = 12콜 × 72 = 864콜/일 < 일일 1000건.
"""
