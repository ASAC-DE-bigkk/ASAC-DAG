...
fetch_raw는 채택한 culture 데이터셋을 KOPIS / 서울 열린데이터 / KOBIS에서 받아 원본
API 응답을 R2 ``raw/culture/`` 아래에 박제만 한다(재현 불가 경계). load_bronze가 그
...
``KOPIS_SERVICE_KEY``, ``SEOUL_API_KEY_CULT``, ``KOBIS_SERVICE_KEY``, ``R2_DEV_*``를
주입) -- 값은 여기 없다.
