...
Discord 임베드 1건. PASS면 초록 색띠, FAIL이면 빨강. **코드블록 표가 아니라 플레인
텍스트 한 줄/데이터셋** — 좁은 화면(모바일)에서 줄이 접혀도 컬럼 정렬이 깨지지 않는다.
(코드블록+컬럼정렬은 한글 double-width + 긴 slug 때문에 모바일에서 줄바꿈되며 무너졌다.)

```
📊 culture raw 적재 리포트 · 2026-07-02 (KST)        [색띠 green/red]
커버리지 12/12 · 5,635행 · 소요 1m33s · freshness 0.3h

✅ 공연목록 `kopis_performance` · 1,204행
✅ 공연상세 `kopis_performance_detail` · 1,180행
   … (데이터셋마다 한 줄) …
⚠️ 시립미술관 전시 `seoul_sema_exhibition` · 18행 — 완전성 미달(<20)
❌ 세종문화회관 공연/전시 `seoul_sejong` — 실패: HTTP 500
```
footer: `수집 00:00:07 → 완료 00:01:58 KST · run_id=… · @daily`

- 한 줄 = `<이모지> <한글 서비스명> \`<slug>\` · <rows>행`. slug 는 **인라인 코드**라
  눈에 띄고 복사·grep 쉬움 → 코드로 바로 점프. 한글명의 엔드포인트/OA 괄호는 제거.
- 상태 = 이모지(✅ ok / ⚠️ WARN / ❌ FAIL / ⏭️ skip). WARN·FAIL 은 **그 줄에 이유 인라인**.
- run 전체 타이밍(수집→완료·소요)은 요약줄/footer 에만. per-dataset pages·완료시각·소요는
  가독성 위해 메시지에서 뺐다(데이터는 run_report JSON 에는 남는다).
- 정상일 땐 색띠 초록, 모든 줄 ✅.
- **조건부 Iceberg**: `total_iceberg_rows > 0`(= write_iceberg 켠 실행)일 때만 요약줄에
  `· Iceberg N` 추가, `rows != iceberg_rows`면 **그 데이터셋 줄이 WARN + 불일치 이유**.
...
{ "embeds": [ {
  ...
  "description": "<요약줄>\n\n<데이터셋마다 한 줄(이모지·한글명·`slug`·rows, 문제면 이유)>",
