...
Cloudflare R2의 `raw/culture/` 경로에 그대로 적재하는 Airflow 파이프라인입니다.
...
| `source/config.py` | 적재 루트 `raw/culture`, KOPIS/서울 API 키 환경변수 로딩·검증. |
...
```
raw/culture/<소스>/<데이터셋>/load_date=<KST날짜>/ingest_ts=<UTC시각>/page-NNNN.<xml|json>
                                                                       /_manifest.json
```
...
```
raw/culture/_reports/load_date=<KST>/ingest_ts=<UTC>/run_report.json
```
