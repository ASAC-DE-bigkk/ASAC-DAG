...
```text
   KOPIS API (XML)          서울 열린데이터 (JSON)
        ...
        └───────────┬─────────────┘
                    ▼
     culture_bronze_ingest DAG            (Airflow · @daily)
     plan → ingest_dataset(×12 동적매핑) → report
                    │
        ┌───────────┴────────────┐
        ▼                        ▼
   R2 raw (원본 보존)        bronze Iceberg (Trino · 선택)
   bronze/culture/…          iceberg[_dev].culture.bronze_*
                    │
                    ▼
...
```
domains/culture/
├─ culture_bronze_ingest.py        # ⭐ Airflow DAG 엔트리 (유일하게 스캔되는 파일)
...
- 🔒 **쓰기 범위**: `bronze/culture/`(R2) · `iceberg[_dev].culture.bronze_*`(Trino)만. 공유 인프라·타 도메인 스키마는 건드리지 않음.
