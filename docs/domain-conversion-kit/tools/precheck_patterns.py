...
def main():
    ...
    # 방어: 실 D1 실행 전 정적 감사 게이트. 이 도구가 내부표(_keys 등)를 읽는 벡터가 되지
    # 않도록, 스캔한 yml 의 게시 테이블(모델명 ∪ 패턴 d1_table)을 allowlist 로 삼아 그 밖의
    # 테이블을 참조하는 패턴은 **실행하지 않는다**(SELECT/WITH·금지토큰도 함께 검사).
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from pattern_audit import audit_pattern_sql  # kit 공유 감사기(같은 폴더)
    ...
    results = []
    ok = skip = fail = blocked = 0
    for p in pats:
        guard = audit_pattern_sql(p["sql"], allow)
        if guard:   # 감사 위반 → 실행 거부(프로덕션 D1 보호)
            results.append({"pattern_id": p["pattern_id"], "status": "blocked", "reason": guard[:2]})
            blocked += 1
            continue
    ...
    report = {"total": len(pats), "ok": ok, "skip": skip, "fail": fail, "blocked": blocked,
              "results": results}
    if args.out:
        Path(args.out).write_text(json.dumps(report, ensure_ascii=False, indent=1), encoding="utf-8")
    print(json.dumps({k: report[k] for k in ("total", "ok", "skip", "fail", "blocked")}, ensure_ascii=False))
    return 0 if (fail == 0 and blocked == 0) else 1
