...
| 2026-07-03 | (#78) 서울 base URL env 이름을 루트 `.env` 이름 `SEOUL_OPEN_API_BASE_URL`로 통일 — commerce 구 이름(`SEOUL_OPENAPI_BASE_URL`) 코드 훅·문서·테스트 예시 개명 | #72 키 통일과 같은 원칙(루트 .env 일원 관리) — 사용자 결정. 실환경 이관 불필요(#72 때 .env.commerce 항목 이미 삭제, 코드 훅만 잔존) |
