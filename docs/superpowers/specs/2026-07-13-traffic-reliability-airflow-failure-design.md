...
- 오류 요약은 Airflow task log와 기존 공통 R2 problem 문서를 우선 사용한다. R2 problem 문서의 redacted `title/detail`을 길이 제한해 표시하고, 읽지 못하면 `원인 미확인`으로 표시한다. exception 전문이나 secret은 출력하지 않는다.
